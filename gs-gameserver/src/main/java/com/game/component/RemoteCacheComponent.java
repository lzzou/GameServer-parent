...
public class RemoteCacheComponent extends AbstractComponent {
    ...
    public boolean initialize() {
        try {
            List<Class<?>> allClasses = ClassUtil.getClasses(GlobalConfigComponent.getConfig().cache.packages);
    ...
    public boolean start() {
        try {
            int interval = GlobalConfigComponent.getConfig().cache.syncInterval;
            if (interval <= 0) {
                interval = 180;
            }
